...
const radius = Math.trunc((Math.min(window.innerWidth, window.innerHeight) * 0.7) / 2);
...
class Point {
  ...
  update(s, offset, pointCount, bin) {
    ...
    switch (bin) {
      case "highs":
        waveFreq = 10;
        beatMultiplier = 100; // Highs get moderate amplification
        break;
      case "mids":
        waveFreq = 6;
        beatMultiplier = 90; // Mids get strong amplification
        break;
      case "lows":
        waveFreq = 2;
        beatMultiplier = 95; // Bass gets maximum amplification for punch
        break;
      default:
        waveFreq = 5;
        beatMultiplier = 60;
    ...
    if (offsetValue > 0.6) { // Strong signal detected
      ...
      beatBoost = 1.0; // 20% extra amplification
    }
    
    // Apply exponential scaling for more dramatic effect on loud parts
    let scaledOffset = Math.pow(offsetValue, 0.7) * (beatMultiplier * 0.5) * beatBoost;
